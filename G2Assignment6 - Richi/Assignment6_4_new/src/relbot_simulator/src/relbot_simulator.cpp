...
{
  // Left motor velocity is inverted relative to right motor in the simulator model.
...
cv::Mat RELbotSimulator::CreateCVSubimage(const sensor_msgs::msg::Image::SharedPtr msg_cam_img, const int center_pixel_x, const int center_pixel_y, int output_image_dim)
{
  ...
  // Use the input image size so the simulator publishes a full-size camera stream.
  const int output_height = std::clamp(output_image_dim, 1, size.height);
  const int output_width = std::clamp((int)(output_image_dim * (double)size.width / (double)size.height), 1, size.width);

  int leftmost_pixel_x = center_pixel_x - output_width / 2;
  int topmost_pixel_y = center_pixel_y - output_height / 2;

  // Make edges 'stick, such that we dont get out of bounds
  leftmost_pixel_x = std::clamp(leftmost_pixel_x, 0, size.width - output_width);
  topmost_pixel_y = std::clamp(topmost_pixel_y, 0, size.height - output_height);

  // Define the rectangle of interest (pixel location + output width/height for zoom)
  cv::Rect rect = cv::Rect(leftmost_pixel_x, topmost_pixel_y, output_width, output_height);
  // turn that into valid sub-image
  cv::Mat subImg = cv_frame(rect);
  // resize into the original camera resolution to keep the published image full-size
  cv::resize(subImg, resized_frame, cv::Size(size.width, size.height), cv::INTER_LINEAR);
  ...
  if (display != nullptr && display[0] != '\0') {
    try {
      ...
      RCLCPP_WARN(get_logger(), "Unable to show simulator image window: %s", e.what());
    }
  } else {
    RCLCPP_DEBUG(get_logger(), "DISPLAY is not set; skipping sim image window.");
  }
